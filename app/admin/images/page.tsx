...
export default function ImagesPage() {
  ...
  async function fetchImages() {
    ...
    // fetch captions for these images
            ...
            {images.map(image => (
              ...
              >
                ...
                <div style={{ padding: '12px 14px', borderBottom: '1px solid #f0f0f0' }}>
                  <p style={{ fontSize: 10, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#aaa', margin: '0 0 6px' }}>
                    Captions
                  </p>
                  {(captions[image.id] ?? []).length === 0 ? (
                    <p style={{ fontSize: 12, color: '#ccc', margin: 0 }}>No captions</p>
                  ) : (
                    (captions[image.id] ?? []).map(c => (
                      <p key={c.id} style={{ fontSize: 12, color: '#555', margin: '0 0 4px' }}>
                        {c.content}
                      </p>
                    ))
                  )}
